...
- TAKE-PROFIT (BACKSTOP): the MOMENT an entry fills, immediately place a resting SELL-to-CLOSE LIMIT for the FULL filled quantity at fillPremium × (1 + ${PARAMS.targetPct}/100), rounded to the nearest cent, time_in_force "gfd". This is a HIGH backstop that banks a fast spike if the code watcher misses it; note the order id so it can be cancelled later. The PRIMARY exit is a MOMENTUM-STALL handled in code (watch.mjs): a fast loop closes the position when SPY stops making new highs (calls) / new lows (puts) for a few minutes — riding the trend and exiting near the peak. You do NOT run that yourself; just place the backstop + the hard stop, and never manually cap a winner early at a small gain.
...
async function runScan(instruction = null, opts = {}) {
  ...
  const prompt = instruction || [
    ...
    `Place a resting +${PARAMS.targetPct}% BACKSTOP limit AT ENTRY (cancel before any stop/EOD close — never double-sell); the code watcher takes profit on a momentum stall. Cut losers at ${PARAMS.stopPct}%. Close all SPY 0DTE by 3:45 PM ET.`,
